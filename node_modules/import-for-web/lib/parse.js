...
let mainIndex = path.join(base,"/",packageJSON.main);
...
let unpackWebMap = `{\n  '/modules/${appName}@${version}': path.resolve(\`\${__dirname}\`,'./${packageJSON.main}'),\n`
...
function parseDependencies(code, filePath, currentDirectory) {
            ...
            for (let i = declarations.length-1; i >=0 ; i--) {
                ...
                if (/^\./.test(dependencyPath)) {
                    ...
                    if (!dependencyMap[dependencyPath]) {
                        ...
                        dependencyMap[dependencyPath] = filename = modulesDirectoryPath + raw;
    ...
    if (!dependencyMap[filePath]) {
        dependencyMap[filePath] = pathname = modulesDirectoryPath + rawFilePath;
