      ...
      for (i = 0; i < atrris.length; i++) {
        ...
        if (typeof  v== "string") {
          if (v.startsWith("{") && v.endsWith("}")) {
            ...
            //console.log([atrris[i],node.attr[atrris[i]]]);
          ...
          // node.attr[atrris[i]] = node.attr[atrris[i]]
          //   .replace(/^./, "")
          //   .replace(/.$/, "");
        } else {
          nodeString = nodeString.replace(
    ...
    for (i = 0; i < ch_l; i++) {
      if (typeof node.children[i] === "string") {
        ...
        if (node.children[i].JS) {
          ...
          //   ...depth,
          //   i,
          // ])},method:${node.children[i].value},index:${JSON.stringify(i)}},`;
          // kNdN.DN.push({
          //   position: [...depth, i],
          //   method: node.children[i].value,
          //   index: i,
          // });
        } else {
          //A static element
           ...
           if (/on[A-Z]\S+/.test(k)) {
            k = k.replace(/on/, "");
             ...
             if (k == "load") {
              ...
              nodeString = nodeString.replace(
                `${attr_Rep}`,
                ` on${k}="Breaker.select(this,'${k}')"${attr_Rep}`
              );
             }
           } else {
             if (k.startsWith("$")) {
